...
function onLoad() {
	...
	locip = localStorage.getItem('locIp');
		if (!locip) {
			locip = prompt("Mode fichier. Entrez l'IP de WLED!");
...
function showErrorToast() {
	showToast('La connexion à la lumière a échoué!', true);
}
...
function getRuntimeStr(rt)
{
	...
	var str = days ? (days + " " + (days == 1 ? "jour" : "jours") + ", ") : "";
	str += (hrs || days) ? (hrs + " " + (hrs == 1 ? "heure" : "heures")) : "";
	if (!days && hrs) str += ", ";
...
function checkUsed(i) {
	...
	if (pJson[id] && (i == 0 || id != i)) {
		d.getElementById(`p${i}warn`).innerHTML = `&#9888; Remplacement ${pName(id)}!`;
	} else {
...
function presetError(empty)
{
	...
	if (empty)
		cn += `Vous n'avez pas encore de presets!`;
	else
		cn += `Désolé, il y a eu un problème lors du chargement des presets!`;

	if (hasBackup) {
		cn += `<br><br>`;
		if (empty)
			cn += `Cependant, il y a une archive des données des presets provenant d'une installation précédente.<br>
			(Sauvegarder un preset effacera ce message et remplacera l'archive)`;
		else
			cn += `Voici une archive des derniers réglages dans un état correct:`;
		cn += `<textarea id="bck"></textarea><br>
			<button class="btn btn-p" onclick="cpBck()">Copier vers le presse-papier</button>`;
	}
	cn += `</div>`;
...
function populateQL()
{
	var cn = "";
	if (pQL.length > 0) {
		cn += `<p class="labels">Chargement rapide</p>`;
		...
		cn += `<p class="labels">Tout les presets</p>`;
	}
...
function populateInfo(i)
{
	...
	${urows}
	${inforow("Build",i.vid)}
	${inforow("Force du Signal",i.wifi.signal +"% ("+ i.wifi.rssi, " dBm)")}
	${inforow("Temps de fonctionnement",getRuntimeStr(i.uptime))}
	${inforow("Heap libre",heap," kB")}
  	${inforow("Intensité estimée",pwru)}
  	${inforow("Frames / secondes",i.leds.fps)}
	${inforow("{{Adresse MAC}}",i.mac)}
	${inforow("Système de fichier",i.fs.u + "/" + i.fs.t + " kB (" +Math.round(i.fs.u*100/i.fs.t) + "%)")}
	${inforow("Environnement",i.arch + " " + i.core + " (" + i.lwip + ")")}
	</table>`;
...
function populateSegments(s)
{
	...
	for (var y = 0; y < (s.seg||[]).length; y++)
	{
		...
		cn += `<div class="seg">
			...
			<div class="segin ${expanded[i] ? "expanded":""}" id="seg${i}">
				<input type="text" class="ptxt stxt noslide" id="seg${i}t" autocomplete="off" maxlength=32 value="${inst.n?inst.n:""}" placeholder="Entrez le nom..."/>
				<div class="sbs">
				...
				<table class="infot">
					<tr>
						<td class="segtd">LED de départ</td>
						<td class="segtd">${cfg.comp.seglen?"Longueur":"LED de fin"}</td>
						<td class="segtd">Décalage</td>
					</tr>
					<tr>
				...
				<table class="infot">
					<tr>
						<td class="segtd">Groupement</td>
						<td class="segtd">Espacement</td>
						<td class="segtd">Appliquer</td>
					</tr>
					<tr>
	...
	if (lowestUnused >= maxSeg) {
		d.getElementById('segutil').innerHTML = '<span class="h">Nombre de segments maximal atteint.</span>';
		noNewSegs = true;
	} else if (noNewSegs) {
		resetUtil();
		noNewSegs = false;
...
function populatePalettes(palettes)
{
	...
	var html = `<div class="searchbar"><input type="text" class="search" placeholder="Cherchez" oninput="search(this)" />
<i class="icons search-icon">&#xe0a1;</i><i class="icons search-cancel-icon" onclick="cancelSearch(this)">&#xe38f;</i></div>`;
	for (let i = 0; i < palettes.length; i++) {
		html += generateListItemHtml(
			'palette',
...
function populateNodes(i,n)
{
  ...
  var nnodes = 0;
	...
	if (i.ndc < 0) cn += `La liste des instances est déactivée.`;
	else if (nnodes == 0) cn += `Aucune autre instance trouvée.`;
	cn += `<table class="infot">
${urows}
${inforow("Instance actuelle:",i.name)}
</table>`;
...
function loadNodes()
{
	...
	.then(res => {
		if (!res.ok) {
			showToast('Impossible de charger la liste des noeuds!', true);
		}
		return res.json();
	})
	.then(json => {
...
function displayRover(i,s)
{
	...
	d.getElementById('lv').innerHTML = `WLED reçoit en live ${i.lm} données${sour}`;
...
function readState(s,command=false) {
  ...
  var i=s.seg[selc];
  if (!i) {
    showToast('Pas de segments!', true);
    updateUI();
    return;
  }
  ...
  if (s.error && s.error != 0) {
    ...
    switch (s.error) {
      case 10:
        errstr = "Impossible de monter le système de fichier!";
        break;
      case 11:
        errstr = "Espace insuffisant pour sauvegarder le preset!";
        break;
      case 12:
        errstr = "Preset non trouvé.";
        break;
      case 19:
        errstr = "Une erreur sur le système de fichier vient d'arriver.";
        break;
      }
    showToast('Erreur ' + s.error + ": " + errstr, true);
  }
  updateUI();
}
...
function toggleNl() {
	...
	if (nlA)
	{
		showToast(`Minuterie activée. Votre lumière va ${nlTar > 0 ? "s'allumer":"s'éteindre"} ${nlMode ? "au-dessus de":"après"} ${nlDur} minutes.`);
	} else {
		showToast('Minuterie déactivée.');
	}
	var obj = {"nl": {"on": nlA}};
	requestJson(obj);
}
...
function toggleSync() {
	...
	if (syncSend)
	{
		showToast('Les autres lumières sur le réseau vont maintenant se synchroniser avec celle-ci.');
	} else {
		showToast('Cette lumière et les autres lumières sur le réseau ne sont plus synchronisées.');
	}
...
function makeSeg() {
	...
	<div class="segname newseg">
		Nouveau segment ${lowestUnused}
		<i class="icons edit-icon expanded" onclick="tglSegn(${lowestUnused})">&#xe2c6;</i>
	</div>
	<br>
	<div class="segin expanded">
		<input type="text" class="ptxt stxt noslide" id="seg${lowestUnused}t" autocomplete="off" maxlength=32 value="" placeholder="Entrez le nom..."/>
		<table class="segt">
			<tr>
				<td class="segtd">LED de départ</td>
				<td class="segtd">${cfg.comp.seglen?"Length":"LED de fin"}</td>
			</tr>
			<tr>
...
function resetUtil() {
	var cn = `<button class="btn btn-s btn-i" onclick="makeSeg()"><i class="icons btn-icon">&#xe18a;</i>Ajouter un segment</button><br>`;
	d.getElementById('segutil').innerHTML = cn;
}
...
<div id="pl${i}o1" style="display:${rep?"block":"none"}">
	<div class="c">Répétez <input class="noslide" type="number" id="pl${i}rp" oninput="plR(${i})" max=127 min=0 value=${rep>0?rep:1}> fois</div>
	Fin du preset:<br>
...
<button class="btn btn-i btn-p" onclick="testPl(${i}, this)"><i class='icons btn-icon'>&#xe139;</i>Test</button>`;
	...
	Inclure la luminosité
	<input type="checkbox" id="p${i}ibtgl" checked>
	<span class="checkmark schk"></span>
</label>
<label class="check revchkl">
	Sauvegarder les limites du segment
	<input type="checkbox" id="p${i}sbtgl" checked>
	<span class="checkmark schk"></span>
</label>`;

	return `<input type="text" class="ptxt noslide" id="p${i}txt" autocomplete="off" maxlength=32 value="${(i>0)?pName(i):""}" placeholder="Entrez le nom..."/><br>
<div class="c">Texte Chargement rapide: <input type="text" class="qltxt noslide" maxlength=2 value="${qlName(i)}" id="p${i}ql" autocomplete="off"/></div>
<div class="h">(Laissez vite pour ne pas avoir de bouton de chargement rapide)</div>
<div ${pl&&i==0?"style='display:none'":""}>
	<label class="check revchkl">
		${pl?"Afficher l'éditeur de playlist":(i>0)?"Remplacer par l'état":"Utilisez l'état actuel"}
...
<div class="c">Sauvegarder l'ID <input class="noslide" id="p${i}id" type="number" oninput="checkUsed(${i})" max=250 min=1 value=${(i>0)?i:getLowestUnusedP()}></div>
<div class="c">
	<button class="btn btn-i btn-p" onclick="saveP(${i},${pl})"><i class="icons btn-icon">&#xe390;</i>Sauvegarder ${(pl)?"playlist":(i>0)?"changes":"preset"}</button>
...
function makePlUtil() {
  if (pNum < 2) {
    showToast("Vous avez besoin d'au moins 2 presets pour créer une playlist!"); return;
  }
	if (plJson[0].transition[0] < 0) plJson[0].transition[0] = tr;
  d.getElementById('putil').innerHTML = `<div class="seg pres">
  <div class="segname newseg">
    Nouvelle liste de lecture</div>
...
function resetPUtil() {
	var cn = `<button class="btn btn-s btn-i" onclick="makePUtil()"><i class="icons btn-icon">&#xe18a;</i>Créer un preset</button><br>
...
function delSeg(s){
	if (segCount < 2) {
		showToast("Vous avez besoin de plusieurs segments pour en effacer un!");
		return;
	}
	expanded[s] = false;
	segCount--;
...
function saveP(i,pl) {
	...
	if (!d.getElementById(`p${i}cstgl`).checked) {
		...
		try {
			...
			obj.win = raw;
			if (raw.length < 2) {
				d.getElementById(`p${i}warn`).innerHTML = "&#9888; S'il vous plait entrer la commande de l'API pour commencer";
				return;
			} else if (raw.indexOf('{') > -1) {
				d.getElementById(`p${i}warn`).innerHTML = "&#9888; Erreur de syntaxe dans la commande de l'API JSON personnalisée";
				return;
			} else if (raw.indexOf("Please") == 0) {
				d.getElementById(`p${i}warn`).innerHTML = "&#9888; S'il vous plait recharger la page pour modifier ce preset";
				return;
			}
		}
		obj.o = true;
	...
	if (obj.o) {
		...
		pJson[pI] = {"n":pN, "win":"S'il vous plait recharger la page pour voir la commande sauvegarder récemment."};
...
function delP(i) {
	...
	if (bt.dataset.cnf == 1) {
		...
		bt.style.color = "#f00";
		bt.innerHTML = "<i class='icons btn-icon'>&#xe037;</i>Confirmez la suppression";
		bt.dataset.cnf = 1;
	}
}
...
function cnfReset()
{
	if (!cnfr)
	{
		...
		bt.style.color = "#f00";
		bt.innerHTML = "Confirmez le redémarrage";
		cnfr = true; return;
	}
...
function rSegs()
{
	...
	{
		bt.style.color = "#f00";
		bt.innerHTML = "Confirmez le reset";
		cnfrS = true; return;
	}
	cnfrS = false;
	bt.style.color = "#fff";
	bt.innerHTML = "Remettre à zéro les segments";
